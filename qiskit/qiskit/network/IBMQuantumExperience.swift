...
public final class IBMQuantumExperience {
    ...
    public func create_user(name: String,
        ...
        self.getRequest() { (req,error) -> Void in
            ...
            req!.post(path: "users/createByAdmin", data: data) { (out, error) -> Void in
                if error != nil {
                    responseHandler([:], error)
                    return
                }
     ...
     */
    public func get_user_groups(access_token: String? = nil,
                                user_id: String? = nil,
                                responseHandler: @escaping ((_:[[String:Any]], _:IBMQuantumExperienceError?) -> Void)) {
        self.getRequest() { (req,error) -> Void in
            if error != nil {
            ...
            req!.get(path: "UserGroups") { (out, error) -> Void in
                if error != nil {
                    responseHandler([], error)
                    return
                }
     ...
     */
    public func set_user_group(id_user: String,
                               name_user_group: String,
                               access_token: String? = nil,
                               user_id: String? = nil,
                               responseHandler: @escaping ((_:[String:Any], _:IBMQuantumExperienceError?) -> Void)) {
        self.get_user_groups(access_token: access_token, user_id: user_id) { (user_groups,error) -> Void in
            if error != nil {
                responseHandler([:], error)
                return
            }
            var id_user_group: String? = nil
            let name_lower = name_user_group.lowercased()
            for group in user_groups {
                if let name = group["name"] as? String {
                    if name.lowercased() == name_lower {
                        if let ident = group["id"] as? String {
                            id_user_group = ident
                            break
                        }
                    }
            ...
            if let ident = id_user_group {
                self.request!.put(path: "users/\(id_user)/groups/rel/\(ident)") { (out, error) -> Void in
                    if error != nil {
                        responseHandler([:], error)
                        return
                    }
                ...
                responseHandler([:],IBMQuantumExperienceError.userGroupError(user_group: name_user_group))
            }
        }
    }
}
